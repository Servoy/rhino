...
public class DBGPStackManager {
    ...
    protected static WeakHashMap<Context, DBGPStackManager> map = new WeakHashMap<>();

    private ArrayList<DBGPDebugFrame> stack = new ArrayList<>();
    ...
    public void enter(DBGPDebugFrame debugFrame) {
        ...
        if (sn != null) {
            ...
            if (hit != null && checkBreakpoint(debugFrame, hit)) suspenOnChangeLine = true;
        }

        // was  if (!suspenOnChangeLine && (getManager().getSuspendOnEntry()) ||
        // (this.toBreakForStepOut != null && getStackFrame(1) == this.toBreakForStepOut))
                ...
                if (hit.expression != null) {
                    ...
                    if (eval != null) {
                        if (eval.equals(Boolean.TRUE)) {
                            needSuspend = true;
                        } else needSuspend = false;
                    } else needSuspend = false;
                } else needSuspend = true;
            }
        }
        return needSuspend;
    }
            ...
            synchronized (this) {
                ...
                while (suspended) {
                    try {
                        ...
                        System.err.print("Script execution stopped " + e.getMessage());
                    }
                }
            }
        }
