...
public class BreakPointManager {
    ...
    private HashMap<String, HashMap<Integer, BreakPoint>> fileMap = new HashMap<>();
    private HashMap<String, BreakPoint> ids = new HashMap<>();
    private HashMap<String, BreakPoint> returnNames = new HashMap<>();
    private HashMap<String, List<BreakPoint>> watchpoints = new HashMap<String, List<BreakPoint>>();
    private HashMap<String, BreakPoint> callNames = new HashMap<>();
    ...
    public synchronized void removeBreakPoint(String id) {
        ...
        if (object != null) removeBreakPoint(object);
    }

    public final synchronized void addBreakPoint(BreakPoint point) {
        if (point.isReturn) {
        ...
        HashMap<Integer, BreakPoint> object =
                (HashMap<Integer, BreakPoint>) fileMap.get(point.file);
        if (object == null) {
            object = new HashMap<>();
            fileMap.put(point.file, object);
        }
        object.put(Integer.valueOf(point.line), point);
    ...
    public synchronized void removeBreakPoint(BreakPoint point) {
        ...
        object.remove(Integer.valueOf(point.line));
        ids.remove("p" + point.id);
    }

    public synchronized BreakPoint hit(String sourcePath, int lineNumber) {
        HashMap<Integer, BreakPoint> q = (HashMap<Integer, BreakPoint>) fileMap.get(sourcePath);
        ...
        if (point.hitValue > 0) {
            ...
            if (point.hitCondition == 1) {
                if (point.hitValue >= point.currentHitCount) return null;
            }
            if (point.hitCondition == 2) {
                if (point.hitValue != point.currentHitCount) return null;
            }
            if (point.hitCondition == 3) {
                if (point.currentHitCount % point.hitValue != 0) return null;
            }
        }
        if (point.isTemporary) q.remove(lnNumber);
        return point;
    }

    public synchronized void updateBreakpoint(
            String id,
            String newState,
            String newLine,
            String hitValue,
        ...
        if (p != null) {
            ...
            if (newLine != null) {
                HashMap<Integer, BreakPoint> map =
                        (HashMap<Integer, BreakPoint>) fileMap.get(p.file);
                Integer nl = Integer.valueOf(p.line);
                BreakPoint po = (BreakPoint) map.get(nl);
                if (!Objects.equals(po, p)) {
                    System.out.println("Error");
                } else {
                    map.remove(nl);
                    p.line = Integer.parseInt(newLine);
                    map.put(Integer.valueOf(p.line), p);
                }
            }
            if (hitValue != null) {
