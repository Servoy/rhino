...
final class ContextGetCommand extends DBGPDebugger.Command {
    ...
    void parseAndExecute(String command, Map<?, ?> options) {
        ...
        try {
            ...
            HashSet<Object> doubles = new HashSet<>();
            ...
            if (context == LOCAL_CONTEXT_ID || context == -1) {
                ...
                if (argsValue != null) {
                    this.debugger.printProperty(
                            "arguments", "arguments", argsValue, properties, 0, true);
                }
                ...
                if (scriptable instanceof ContextualScope) {
                    sendAllIds(
                            properties,
                            stackFrame,
                            ((ContextualScope) scriptable).getContextScriptable(),
                            doubles,
                            false);
                }
            }
            ...
            // on.
            System.err.println("Error in ContextGetCommand: " + t.getMessage());
        }
        ...
        if (scriptable != null) {
            ...
            for (int i = 0; i < ids.length; i++) {
                ...
                if (!doubles.contains(id)) {
                    ...
                    {
                        this.debugger.printProperty(
                                id.toString(), id.toString(), value, properties, 0, true);
                    }
                }
            }
            ...
            if (walkParents)
                sendAllIds(
                        properties, stackFrame, scriptable.getParentScope(), doubles, walkParents);
        }
    }
}
