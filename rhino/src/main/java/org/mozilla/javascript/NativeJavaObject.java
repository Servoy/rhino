...
public class NativeJavaObject implements Scriptable, SymbolScriptable, Wrapper, Serializable {
    ...
    public NativeJavaObject(
        ...
        this.parent = ScriptableObject.getTopLevelScope(scope); /** servoy patch */
...
//                break;
            ...
            case JSTYPE_BIGDECIMAL:
                if (to.isPrimitive()) {
                    ...
                    if (to == TypeInfo.STRING) {
                        ...
                        return 9;
                    } else if (to.is(BigInteger.class) || to.is(BigDecimal.class)) {
                        return 10;
                    } else if (to.isObjectExact()) {
                        return 11;
                    } else if (to.isNumber()) {
            ...
            case JSTYPE_STRING:
                ...
                } else if (ScriptRuntime.NumberClass.isAssignableFrom(to.asClass())) {
                    return 4;
                }
                break;
            ...
            case JSTYPE_JAVA_ARRAY:
                ...
                if (to.asClass().isInstance(javaObj)) {
                    return CONVERSION_NONTRIVIAL;
                }
                if (to == TypeInfo.STRING) {
                    return 2;
...
//                    return "undefined";
            ...
            case JSTYPE_BIGDECIMAL: /**BigDecimal patch **/
                if (type == TypeInfo.STRING) {
                    ...
                    return coerceToNumber(
                            jsTypeCode == JSTYPE_BIGINT ? TypeInfo.BIG_INT : ( jsTypeCode == JSTYPE_BIGDECIMAL ? TypeInfo.BIG_DECIMAL : TypeInfo.DOUBLE /**BigDecimal patch **/),
                            value);
            ...
            case JSTYPE_JAVA_ARRAY:
                ...
                if (type.isArray() && value.getClass().isArray()) {
                    ...
                    TypeInfo arrayType = type.getComponentType();
                    Object result = Array.newInstance(arrayType.asClass(), (int) length);
                    for (int i = 0; i < length; ++i) {
                        try {
                            Array.set(result, i, coerceTypeImpl(arrayType, Array.get(value, i)));
